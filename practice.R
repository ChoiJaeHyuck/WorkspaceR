...
# Vector : 1차원 배열
# Vector 요소 : 데이터 과학에서는 차원이라는 용어사용
# Data Frame : 2차원 배열
# Data Frame의 열(변수)을 데이터 과학에서는 차원이라는 용어 사용
...
# : 고차원 공간의 표본을 저차원으로 변환하는 차원 축소 기법

# 2. t-SNE( Stochastic Neighbor Embedding )
# : 단어 벡터와 같이 고차원 데이터를 시각화하는데 사용하는 차원 축소 기법
# : 고차원 공간에 존재하는 데이터 x의 이웃 간의 거리를 최대한 보존하는 저차원 y를 학습하는 방법
...
# 데이터 과학에서 독립변수( independent variable ), 설명변수( explanatory variable )
# 특징( feature ) : -> X라 하고
# 
# 종속변수( dependent variable ), 반응변수( response variable ),레이블( label, 정답 ) -> y라 할 때,
# 
# X가 입력되면 y를 맞추어야 하는 문제, y를 ground truth로 간주 

# 회귀분석( Regression Analysis )
# : 회귀 이론을 기초로 독립변수 X가 종속변수 y에 미치는 영향을 파악하여 에측 모델을 도출하는 통계적인 방법

# 예측 모델을 도출할 때, 회귀 이론에 기초하기 때문에 회귀 모델이라함.

# 회귀 모델에서는 독립변수와 종속변수 사이의 관계를 수학식의 형태로 표현하는데 
# 이를 회귀식( regression equation )이라 한다.

# 회귀 모델에서 독립변수는 X, 종속변수는 y로 표기
# 회귀 모델을 통하여 종속변수를 예측할 수 있는 모델이다.

# 단순선형 회귀모델( simple linear regression analysis )
# : 독립변수 X와 종속변수 y사이의 선형 관계를 파악하고 이를 예측하는 통계적 방법

# 독립변수 X와 종속변수 y와의 관계를 선형으로 표현. 하나의 독립변수를 다루는 분석방법


# 단순선형 회귀모델의 회귀식 : y = WX + b( W, b)        X : 독립변수 y : 종속변수
...
# -> 수집된 데이터 X, y로 이용해 산점도 작성
# -> 회귀식은 산점도 상에서 X, y의 추세를 나타내는 선에 대한 식
# -> 현재 수집된 데이터의 추세를 가장 잘 반영할 수 있는 W, b를 찾는 것이 단순 선형 회귀 모델의 목표
...
deviance( model ) / length( cars$speed )         # 잔차 제곱합을 평균 제곱 오차( MES, mean squared error )로 변환

# 귀무가설( null hypothesis ) : 처음부터 버릴 것을 예상하는 가설
# 대립가설( alternative hypothesis ) : 귀무가설에 대립하는 명제. 모집단에서 독립변수와 종속변수 사이에
#                                      어떤 특정한 관련이 있다는 꼴이다
...
# : 데이터의 크기는 평균은 클수로, 분산은 작을수록, 데이터의 크기가 클수록 믿음이 커진다에서 이 세 가지 값을 묶은것

# p-값( p-value ) : 귀무가설을 가정했을 때, 어떤 통계량이 관측 데이터와 같거나 클 확률

# t-값이 크면 대립가설에 대한 믿음이 강해지고 귀무가설에 대한 믿음이 약해진다
# t-값이 작으면 대립가설에 대한 믿음은 약해지고 귀무가설에 대한 믿음은 강해진다.

# 데이터를 통해 ' 대립가설이 통계학적으로 유의미하다'라는 것을 증명하고 확인하는 작업을 t-검정(t-test)이라 한다.

# '귀무가설이 참이라고 가정했을 때, 표본으로부터 얻어지는 통계치가 나타날( 관측될 ) '확률'을 계산하는데
# 이 때, 계산된 확률값을 p값이라 함. p값이 매우 낮으면, 이러한 표본 통계값은 우연히 나타나기 어려운 케이스이기 떄문에
...
# 다중선형 회귀모델 : 여러개의 독립변수를 다루는 회귀모델. 어떤 결과에 영향을 미치는 요인이 하나이기 보다는
#                     여러 개인 경우가 대부분이기 때문에 다중선형 회귀분석( 모델 )에 대한 이해는 중요

# 회귀식 : y = W1X1 + W2X2 + ... + WnXn + b

# 독립변수가 n개인 다중선형 회귀에서 주어진 자료를 이용해 W!, W2, ..., Wn, b의 값을 알아내는 회귀모델
...
# 다중 선형 회귀모델에서 변수 선택
# : 종속변수를 설명하는 데 도움이 되는 독립변수가 다수 존재한다. 그런데 모든 독립변수가 종속변수를
#   설명하는데 동일하게 기여하는 것은 아니다. 어떤 변수는 기여도가 높고 어떤 변수는 낮다.

# 기여도가 낮거나 거의 없는 변수들은 모델에서 제외하는 것이 좋다. 적은 변수를 이용해 현실을 잘 설명할 수 있는
# 것이 좋은 모델이기 때문이다.
...
# 2.2 로지스틱 회귀분석( logistic regression analysis ) ---> 예측 결과가 실수가 나온다( 0 ~ 1)

# 회귀모델에서 종속 변수의 값의 형태가 범주형 값인 경우를 다루기 위해서 만즐어진 통계적 모델

# 주어진 데이터로부터 어떤 범주를 예측하는 분야를 회귀와 구분하여 분류( classification )라 하고
# 분류 문제를 회귀방법으로 해결하고자 개발된 회귀분석

# 로지스틱 회귀도 기본적으로 회귀기법이기 때문에 종속변수는 숫자로 표현되어야 한다.

# 예) YES와 NO는 0과 1로 변환
# 예) setosa, versicolor, verginica는 1,2,3과 같이 숫자로 바꾼 후에 로지스틱 회귀 적용
...
pred
round( pred, 0)                                      # 예측결과( 소수 첫째 자리에서 반올림 )

# 예측값에 대한 실제 품종 이름 알아보기

pred <- round( pred, 0 )

levels( iris$Species )[ pred ]                # levels() : 펙터 타입의 자료에 대해 어떤 종류 값인지 출력
